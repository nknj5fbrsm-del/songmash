...
export function getBerlinWeekNumber(date: Date): number {
  const p = getBerlinParts(date)
  const monday = addBerlinDays(p.year, p.month, p.day, -(p.isoWeekday - 1))
  const jan4 = berlinNoonUtc(monday.year, 1, 4)
  const jan4Parts = getBerlinParts(jan4)
  const week1Monday = addBerlinDays(jan4Parts.year, jan4Parts.month, jan4Parts.day, -(jan4Parts.isoWeekday - 1))
  const mondayNoon = berlinNoonUtc(monday.year, monday.month, monday.day)
  const week1Noon = berlinNoonUtc(week1Monday.year, week1Monday.month, week1Monday.day)
  const diffDays = Math.round((mondayNoon.getTime() - week1Noon.getTime()) / 86_400_000)
  return Math.floor(diffDays / 7) + 1
}

export function formatWeekCountdown(msRemaining: number): string {
