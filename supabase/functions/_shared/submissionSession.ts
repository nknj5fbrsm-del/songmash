...
export async function createSubmissionSession(secret: string): Promise<string> {
  const payload = {
...
export async function verifySubmissionSession(token: string, secret: string): Promise<boolean> {
...
export function requireSubmissionSession(req: Request): string | null {
  return req.headers.get('x-submission-session')?.trim() || null
}
